...
| S33 | Rootly AI Labs `logs-dataset` (Apache-2.0) — https://huggingface.co/datasets/Rootly-AI-Labs/logs-dataset           | Real production access + error logs. Anchors the ArtifactStore (Issue #27, ADR-17). The "no other team has real production data" moat from `FlawsToProduction/The Situation First.md`. |
| S34 | SRE-skills-bench (public benchmark of SRE tasks with ground-truth answers)                                         | Cross-validation eval to prove agents trained on Praxis transfer to real SRE tasks. Stretch deliverable — referenced from README as future work, not blocking.       |
| S35 | Unsloth GRPO docs + multi-turn GRPO (mtGRPO) — turn-level credit assignment for multi-turn agent trajectories      | Justifies the trainer choice in ADR-19 / Issue #31. ~2.5× throughput vs plain GRPO; stable gradients on sparse-reward long-horizon tasks.                            |
| S36 | UltraHorizon long-horizon agent benchmark + failure-mode taxonomy                                                  | Cited in MissionOps design (ADR-16). Validates the Theme #2 capability gap: planning + memory + recovery + scattered instructions over 80–300 turns.                  |
| S37 | `FlawsToProduction/Critical Mistakes (Real-World Failures).md`                                                     | The 10-mistake architectural audit driving Issues #22 (safety bundle), #23 (outcome reward), #24 (rubrics), #26 (MissionOps), #33 (rollout). Primary source of truth for the rewrite. |
| S38 | `FlawsToProduction/Verdict.md` ("Praxis MissionOps" feature spec)                                                  | The MissionOps feature spec — 4-layer (Goal / Planning / Execution / Audit), 4-rubric reward, scattered instructions across artifacts. Anchors ADR-16, ADR-18.       |
| S39 | `FlawsToProduction/The Situation First.md` (48-hour winning plan)                                                  | The 48-h timeline + Rootly recommendation. Anchors ADR-17, ADR-19; informs `idea/Plan/implementation_plan.md` v3.0 schedule.                                          |
| S40 | User judge-checklist (4-axis 40/30/20/10 with 20 verbatim items)                                                   | The submission tracker's source of truth. Anchors `idea/Plan/Submission/SubmissionChecklist.md` and Issue #39.                                                       |
