"""
praxis_env.scenarios.mega_incident_legacy - Task 5 legacy core (pre-MissionOps).

This module hosts the original Mega-incident scenario as
``MegaIncidentScenarioLegacy`` so that ``mission_scenario.MissionScenario``
can subclass it without touching the legacy data tables. The historical
public name ``MegaIncidentScenario`` is re-exported from
``praxis_env.scenarios.mega_incident`` and now points at ``MissionScenario``
(Issue #37). Tests that need the legacy 120-step shape import this module
directly.
"""
...
class MegaIncidentScenarioLegacy(BaseScenario):
    """Legacy 120-step Mega-incident scenario (kept for back-compat)."""
